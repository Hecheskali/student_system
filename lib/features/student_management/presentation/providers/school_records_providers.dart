    ...
    Provider<HistoricalRecordsOverview>((Ref ref) {
      ...
      return HistoricalRecordsOverview(
        ...
        nationalSessions: state.examSessions
            .where(
              (ExamSession session) => session.type == ExamSessionType.national,
            )
            .length,
        mockSessions: state.examSessions
            .where(
              (ExamSession session) => session.type == ExamSessionType.mock,
            )
            .length,
...
final Provider<List<StudentPrediction>> studentPredictionsProvider =
    Provider<List<StudentPrediction>>((Ref ref) {
      ...
      final List<StudentPrediction> predictions =
          currentResults.map((StudentResultRecord result) {
            final List<HistoricalExamRecord> matches =
            ...
            final List<SubjectResult> orderedSubjects =
                <SubjectResult>[...result.subjectResults]..sort(
...
final Provider<List<TeacherProjection>>
teacherProjectionsProvider = Provider<List<TeacherProjection>>((Ref ref) {
  final SchoolRecordsState records = ref.watch(schoolRecordsProvider);
    ...
    final double currentAverage = assignedStudents.isEmpty
        ...
        : _average(
            assignedStudents.map(
              (StudentResultRecord result) => result.subjectResults
                  .firstWhere(
    ...
    final double historicalAverage = historicalMatches.isEmpty
        ...
        : _average(
            historicalMatches.map(
              (HistoricalExamRecord record) => record.result.subjectResults
                  .firstWhere(
                    (SubjectResult subject) =>
                        subject.subject == teacher.subject,
                  )
...
final Provider<List<RecommendationInsight>>
recommendationInsightsProvider = Provider<List<RecommendationInsight>>((
  Ref ref,
) {
  ...
  final List<RecommendationInsight> recommendations = <RecommendationInsight>[
    if (predictions.isNotEmpty)
      RecommendationInsight(
        ...
        metricValue: '${predictions.last.predictedAverage.toStringAsFixed(1)}%',
...
class SchoolRecordsController extends StateNotifier<SchoolRecordsState> {
  ...
  void importHistoricalRecords({
    ...
    final List<HistoricalExamRecord>
    importedRecords = results.asMap().entries.map((
